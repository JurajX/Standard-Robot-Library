...
protected:
    using FP = double;
    const static inline srl::types::QuatVec3<FP> basePose { srl::types::Quat<FP>::Identity(), srl::types::Vector3<FP>::Zero() };
    const static inline srl::types::Vector3<FP> gravityDirection { 0, 0, -1 };
    // NOLINTNEXTLINE
...
#endif
    const static inline uint32_t nJoints { pendulum::single::numLinks };
    const static inline std::vector<srl::types::VectorX<FP>> qSet {
        srl::utils::getRandVectors<FP>(size, nJoints, -srl::constants::kPI<FP>, srl::constants::kPI<FP>)
    };
    const static inline std::vector<srl::types::VectorX<FP>> dqSet {
        srl::utils::getRandVectors<FP>(size, nJoints, -srl::constants::kPI<FP>, srl::constants::kPI<FP>)
    };

    const static inline srl::types::Vector3<FP> com { pendulum::single::linkCoM<FP>.col(0) };
    const static inline srl::types::Vector3<FP> rotAxis { pendulum::single::jointRotAxes<FP>.col(0) };
...
TYPED_TEST(TestSinglePendulum, PhysicalAroundCom)    // NOLINT
{
    ...
    using FP = TypeParam;
    this->model.setLinkCentresOfMasses(
      srl::types::Matrix3X<FP>::Zero(srl::constants::kDim, this->nJoints));    // rotate around CoM -> set CoM to zeros;
...
TYPED_TEST(TestSinglePendulum, Physical)    // NOLINT
{
    ...
    for (size_t idx = 0; idx < this->size; idx += 1) {
        ...
        const FP Vtot { this->mass * srl::constants::kG<FP> * rotatedCom };
...
protected:
    ...
    const static inline size_t size { 10'000 };
#endif
    const static inline uint32_t nJoints { pendulum::dual::numLinks };
    const static inline std::vector<srl::types::VectorX<FP>> qSet {
        srl::utils::getRandVectors<FP>(size, nJoints, -srl::constants::kPI<FP>, srl::constants::kPI<FP>)
    };
    const static inline std::vector<srl::types::VectorX<FP>> dqSet {
    ...
    const static inline std::vector<srl::types::Vector3<FP>> com { pendulum::dual::linkCoM<FP>.col(0), pendulum::dual::linkCoM<FP>.col(1) };

    // NOLINTNEXTLINE
    std::vector<srl::types::Vector3<FP>> rotAxes { pendulum::dual::jointRotAxesPlanar<FP>.col(0), pendulum::dual::jointRotAxesPlanar<FP>.col(1) };
    const static inline std::vector<srl::types::Vector3<FP>> frameCoos { pendulum::dual::frameCoos<FP>.col(1), pendulum::dual::frameCoos<FP>.col(2) };

    const static inline FP half { 0.5 };
    const static inline std::vector<FP> mass { pendulum::dual::masses<FP> };
    // NOLINTNEXTLINE
    std::vector<FP> projectedInertias { rotAxes[0].transpose() * pendulum::dual::inertias<FP>[0] * rotAxes[0],
...
TYPED_TEST(TestDoublePendulum, PhysicalPlanar)    // NOLINT
{
    ...
    for (size_t idx = 0; idx < this->size; idx += 1) {
        ...
        const FP V_1 { this->mass[0] * srl::constants::kG<FP> * com0norm * (-std::cos(qs0[0])) };
        const FP V_21 { this->mass[1] * srl::constants::kG<FP> * fCooNorm * (-std::cos(qs0[0])) };
        const FP V_22 { this->mass[1] * srl::constants::kG<FP> * com1norm * (-std::cos(qs0[0] + qs0[1])) };
...
TYPED_TEST(TestDoublePendulum, PhysicalNonPlanar)    // NOLINT
{
    ...
    const std::vector<srl::types::Matrix3<FP>> inertias { this->model.getLinkInertiaTensors()[0], this->model.getLinkInertiaTensors()[1] };
    srl::types::MatrixX<FP> rotProjInertias { srl::types::MatrixX<FP>::Zero(2, 2) };
    srl::types::Matrix3<FP> tmp;
    FP tmp1 {};
    FP tmp2 {};
    ...
    for (size_t idx = 0; idx < this->size; idx += 1) {
        ...
        const FP V_1 { this->mass[0] * srl::constants::kG<FP> * com0norm * (-std::cos(qs0[0])) };
        const FP V_2 { this->mass[1] * srl::constants::kG<FP> * fCooNorm * (-std::cos(qs0[0])) };
        const FP V_3 { this->mass[1] * srl::constants::kG<FP> * com1norm * (-std::cos(qs0[0]) * std::cos(qs0[1])) };

        const srl::types::Matrix3<FP> R_1 { srl::math::rotation<FP>(qs0[0], this->rotAxes[0]) };
        const srl::types::Matrix3<FP> R_2 { srl::math::rotation<FP>(qs0[1], this->rotAxes[1]) };
        const srl::types::Matrix3<FP> R12 { R_1 * R_2 };
...
#endif
    const static inline FP half { 0.5 };
    const static inline uint32_t nJoints { chain::numLinks };
    srl::types::Map<const srl::types::VectorX<FP>> damping { chain::damping<FP>.data(), static_cast<uint32_t>(chain::damping<FP>.size()) };
    const static inline std::vector<srl::types::VectorX<FP>> qSet {
        srl::utils::getRandVectors<FP>(size, nJoints, -srl::constants::kPI<FP>, srl::constants::kPI<FP>)
    };
...
TEST_F(TestDerivatives, TestMassMatrix)    // NOLINT
{
    ...
    for (size_t idx = 0; idx < size; idx += 1) {
        ...
        std::vector<srl::types::MatrixX<FP>> actual;
        std::transform(massMat_Der.cbegin() + 1, massMat_Der.cend(), std::back_inserter(actual), [](auto &mat) -> srl::types::MatrixX<FP> {
            return mat;
        });
        const srl::types::MatrixX<FP> massMat { massMat_Der.at(0) };
        for (uint32_t iJoint = 0; iJoint < nJoints; iJoint += 1) {
            srl::types::VectorX<FP> h00 { qs0 };
            h00[iJoint] += delta;
            const srl::types::MatrixX<FP> expected { (std::get<0>(model.getMassMatrixAndPotEnergy(h00)) - massMat) / delta };
...
TEST_F(TestDerivatives, TestPotEnergy)    // NOLINT
{
    ...
    for (size_t idx = 0; idx < size; idx += 1) {
        ...
        const srl::types::VectorX<FP> actual { std::get<2>(eomParams).tail(nJoints) };
...
TEST_F(TestDerivatives, TestChristoffelSymbols)    // NOLINT
{
    ...
    for (size_t idx = 0; idx < size; idx += 1) {
        ...
        for (uint32_t i = 0; i < nJoints; i += 1) {
            for (uint32_t j = 0; j < nJoints; j += 1) {
                for (uint32_t k = 0; k < nJoints; k += 1) {
...
TEST_F(TestDerivatives, TestMotorTorques)    // NOLINT
{
    ...
    for (size_t idx = 0; idx < size; idx += 1) {
        const auto &qs0 { qSet[idx] };
        const srl::types::VectorX<FP> actual { model.getMotorTorque(qs0, qs0, qs0) };
        const auto [massMat, chs, pe0] { model.getEomParams(qs0) };
        srl::types::VectorX<FP> expected { massMat[0] * qs0 + pe0.tail(nJoints) + qs0.cwiseProduct(damping) };
        for (uint32_t iJoint = 0; iJoint < nJoints; iJoint += 1) {
...
TEST_F(TestDerivatives, TestAngularAccelerations)    // NOLINT
{
    ...
    for (size_t idx = 0; idx < size; idx += 1) {
        const auto &qs0 { qSet[idx] };
        const srl::types::VectorX<FP> actual { model.getAngularAcceleration(qs0, qs0, qs0) };
        const auto [massMat, chs, pe0] { model.getEomParams(qs0) };
        srl::types::VectorX<FP> expected { qs0 - pe0.tail(nJoints) - qs0.cwiseProduct(damping) };
        for (uint32_t iJoint = 0; iJoint < nJoints; iJoint += 1) {
            expected[iJoint] -= qs0.transpose() * chs[iJoint] * qs0;
        }
